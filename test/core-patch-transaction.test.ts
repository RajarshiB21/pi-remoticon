...
async function assertPatchedToolFlow(chunk: string): Promise<void> {
  ...
  for (const [state, expected] of [["pending", "Reading…"], ["done", "1 line"], ["failed", "failed · broken input"], ["stopped", "interrupted"]] as const) {
    const owner = setup();
  ...
  expect(plain(split)).toContain("failed · failed then continued");
  expect(plain(split)).toContain("Reading…");
