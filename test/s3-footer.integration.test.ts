...
describe("fullscreen composer/footer lifecycle", () => {
  ...
  it("streams, edits a draft, changes effort, resizes and settles without losing native controls", async () => {
    ...
    try {
      ...
      await term.waitFor("Read(package.json)", 5000);
      await term.waitFor("Finished", 5000);
      term.press("Ctrl+O");
      await term.waitFor("offline-tool-fixture", 5000);
      term.press("Ctrl+O");
      await term.waitFor("Read(package.json)", 5000);
      ...
      await term.waitFor("Read(package.json)", 10000);
      await term.waitFor("Finished", 5000);
      ...
      term.press("Ctrl+O");
      term.press("Ctrl+O");
      await term.waitFor("Read(package.json)", 3000);
      await term.waitForStable(100, 3000);
      term.press("Ctrl+T");
